...
export const HeroPage = () => {
  ...
  const { data: heroeResponse, isError } = useQuery({
    queryKey: ["heroe", { idSlug: idSlug }],
  ...
  const averagePower = heroeResponse
    ? Math.round(
        ((heroeResponse.strength +
          heroeResponse.intelligence +
          heroeResponse.speed +
          heroeResponse.durability) /
          4) *
          10,
      )
    : 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="relative">
              <img
                src={heroeResponse?.image || "/placeholder.svg"}
                alt={heroeResponse?.alias}
                width={200}
                height={200}
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                <Badge
                  className={`${getCategoryColor(heroeResponse?.category ?? "")} text-white`}
                >
                  {heroeResponse?.category}
                </Badge>
                <Badge
                  className={`${getStatusColor(heroeResponse?.status ?? "")} text-white`}
                >
                  {heroeResponse?.status}
                </Badge>
                <Badge
                  variant="secondary"
                  className="bg-white/20 text-white border-white/30"
                >
                  {heroeResponse?.universe}
                </Badge>
              </div>
      ...
      <div>
        <CustomBreadcrumbs
          CurrentPage={heroeResponse?.alias ?? ""}
          // breadcrumbs={[
          //   { label: "Home", to: "/" },
          //   { label: "Home2", to: "/" },
          // ]}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <Card className="text-center">
                <CardContent className="pt-6">
                  ...
                  <Progress
                    value={(heroeResponse?.strength ?? 0) * 10}
                    className="h-2"
                  />
                </CardContent>
              </Card>
              ...
              <Card className="text-center">
                <CardContent className="pt-6">
                  ...
                  <Progress
                    value={(heroeResponse?.intelligence ?? 0) * 10}
                    className="h-2"
                  />
                </CardContent>
              </Card>
              ...
              <Card className="text-center">
                <CardContent className="pt-6">
                  ...
                  <Progress
                    value={(heroeResponse?.speed ?? 0) * 10}
                    className="h-2"
                  />
                </CardContent>
              </Card>
              ...
              <Card className="text-center">
                <CardContent className="pt-6">
                  ...
                  <Progress
                    value={(heroeResponse?.durability ?? 0) * 10}
                    className="h-2"
                  />
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {heroeResponse?.powers.map((power, index) => (
                    <div
                      key={index}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b">
                    ...
                    <span className="font-semibold">{heroeResponse?.name}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
